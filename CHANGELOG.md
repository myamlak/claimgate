...
- Every row is registered under a `PropertyType` - what the row is a claim
  about - which is the record of the defect class its cells can see.
  `Book::Property` reads it back; the report counts the rows and cells of each
  type apart from the others, so a type with no rows is visible as a defect
...
- Test suite covering the counter rules, the verdict boundaries, the
  ...
  property types separated by the report, two books in one process,
  out-of-range indices, and the report's wording.
